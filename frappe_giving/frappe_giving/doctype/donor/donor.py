...
class Donor(Document):
	...
	def _create_user(self):
		...
		# Grant the Donor role so the permission hooks can scope the
		# portal's reads (and PDF prints) to this donor's own records.
		existing_roles = {r.role for r in (user.roles or [])}
		if "Donor" not in existing_roles:
			user.append("roles", {"role": "Donor"})
			user.save(ignore_permissions=True)

		self.db_set("user", user.name, update_modified=False)
		frappe.msgprint(_("User {0} linked to Donor").format(user.name), alert=True)
		...
		customer = frappe.get_doc(
			{
				...
				"customer_type": ("Individual" if self.donor_type == "Individual" else "Company"),
			}
		)
	...
	def _create_contact(self, customer_name):
		...
		frappe.msgprint(
			_("Contact {0} created and linked to Donor and Customer").format(contact.name),
			alert=True,
		)
		return contact
