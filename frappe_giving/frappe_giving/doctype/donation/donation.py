...
class Donation(Document):  # nosemgrep: frappe-modifying-but-not-comitting-other-method
	...
	def _create_sales_invoice(self, amount=None, posting_date=None, persist_link=True):
		...
		if not donor.customer:
			frappe.throw(
				_(
					"Donor {0} has no linked Customer. Please save the Donor record first to auto-create one."
				).format(self.donor)
			)
		...
		donation_item = {
			...
			"description": _("Donation {0} – Campaign: {1}").format(self.name, self.campaign),
		}
		if campaign.account:
	...
	def _create_cycle_invoice_and_payment(self, amount, date, stripe_pi_id):
		...
		"""
		si = self._create_sales_invoice(amount=amount, posting_date=date, persist_link=False)
	...
	def _create_erpnext_subscription(self):
		...
		if not donor.customer:
			frappe.throw(
				_("Donor {0} has no linked Customer; cannot create an ERPNext Subscription.").format(
					self.donor
				)
			)
		...
		frappe.msgprint(
			_("ERPNext Subscription {0} created for recurring donation").format(sub.name),
			alert=True,
		)
		return sub
	...
	def _ensure_subscription_plan(self, campaign):
		...
		if not interval:
			frappe.throw(
				_("Frequency {0} is not supported for recurring subscriptions.").format(self.frequency)
			)
