...
def initiate_stripe_payment(form_name, amount, frequency, donor_data):
	...
	customer_id = stripe_helpers.get_or_create_stripe_customer(
		donor, email, full_name
	)
...
def _handle_invoice_payment_succeeded(invoice):
	...
	donation_name = frappe.db.get_value(
		"Donation", {"gate_subscription_id": subscription_id}, "name"
	)
	if not donation_name:
		return
...
def _handle_subscription_updated(sub):
	donation_name = frappe.db.get_value(
		"Donation", {"gate_subscription_id": sub["id"]}, "name"
	)
	if not donation_name:
		return
...
def _handle_subscription_deleted(sub):
	...
	if donation_name:
		frappe.db.set_value(
			"Donation", donation_name, "subscription_status", "Cancelled"
		)
...
def _handle_charge_refunded(charge):
	...
	if not pi_id:
		return
	dp_name = frappe.db.get_value(
		"Donation Payment", {"stripe_payment_intent_id": pi_id}, "name"
	)
	if dp_name:
