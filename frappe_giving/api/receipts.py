...
def generate_yearly_statement(donor_name: str, year: int) -> tuple[str, bytes]:
	...
	for line in lines:
		if line.campaign and line.campaign not in campaign_labels:
			campaign_labels[line.campaign] = (
				frappe.db.get_value("Donation Campaign", line.campaign, "campaign_name") or line.campaign
			)
	...
	primary_company = max(company_counts.items(), key=lambda kv: kv[1])[0] if company_counts else None
	...
	donor_doc.statement_currency_breakdown = [
		frappe._dict(currency=c, total=round(t, 2)) for c, t in sorted(totals_by_currency.items())
	]
	...
	donor_doc.statement_total_usd = round(total_usd, 2) if len(totals_by_currency) > 1 else 0
...
def download_yearly_statement(year: int):
	...
	if not donor_name:
		frappe.throw(_("No donor record linked to your account."), frappe.PermissionError)
