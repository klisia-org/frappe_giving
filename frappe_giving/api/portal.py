...
def get_portal_donate_form() -> dict:
	...
	donor_profile = frappe.db.get_value("Donor", donor, ["donor_name", "email"], as_dict=True) or {}
...
@frappe.whitelist()
def get_donation_history(year: int | None = None, limit: int = 50, offset: int = 0) -> dict:
...
def get_yearly_summary(year: int) -> dict:
	...
	return {
		...
		"line_items": lines,
		"currency_breakdown": [
			{"currency": c, "total": round(t, 2)} for c, t in sorted(totals_by_currency.items())
		],
